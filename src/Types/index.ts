...
export type TLoadingButton = Pick<Props, 'loadingText' | 'textStyle'> & { contentColor: string };

export type TContentButton = Pick<Props, 'iconLeft' | 'iconRight' | 'textStyle'> & { contentColor: string, title: string };

export type TButtonWrapper = Partial<Omit<Props, 'onPress'>> & { onPress: () => void, children?: React.ReactNode };